...
REGISTER_ALGORITHM(Smart3)


void Smart3::setSensor(const AbstractSensor& s)
{
	sensor = &s;
	moreSteps = -1;
...
void Smart3::setConfiguration(map<string, int> config)
{
...
Direction Smart3::step(Direction prevStep)
{
	...
	// - is not recharging atm, should not go back to the docking station, and should pick a new step
		...
		if (si.dirtLevel > 1)
			...
			if (house.hasCell(south) && (house.getCell(south) > '0') && (house.getCell(south) <= '9'))
...
void Smart3::aboutToFinish(int stepsTillFinishing)
{
	moreSteps = stepsTillFinishing;
}

Direction Smart3::oppositeMove(Direction d)
{
